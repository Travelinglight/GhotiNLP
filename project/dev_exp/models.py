...
# weight = [1,1,1,1]
weight = [0.0807432058, 0.5763059608,-0.3216972178,0.70923968,-1.2847693]
phrase = namedtuple("phrase", "english, logprob1, logprob2, logprob3, logprob4")
def TM(filename, k):
  sys.stderr.write("Reading translation model from %s...\n" % (filename,))
  tm = {}
  tmFile = gzip.open(filename, 'r') if filename[-3:] == '.gz' else open(filename, 'r')
  for line in tmFile.readlines():
    (f, e, logprob) = line.strip().split(" ||| ")
    tm.setdefault(tuple(f.split()), []).append(phrase(e, float(logprob.strip().split()[0]),float(logprob.strip().split()[1]),float(logprob.strip().split()[2]),float(logprob.strip().split()[3])))
  for f in tm: # prune all but top k translations
    tm[f].sort(key=lambda x: - weight[0]*x.logprob1 - weight[1]*x.logprob2 - weight[2]*x.logprob3 - weight[3]*x.logprob4)
    del tm[f][k:] 
  return tm
